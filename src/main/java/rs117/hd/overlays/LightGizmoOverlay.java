		...
		for (int i = lights.size() - 1; i >= -1; i--) {
			...
			if (isSelected && !wasCameraReoriented && inverseProjection != null) {
				if (action == Action.GRAB) {
					...
					for (int j = 0; j < 3; j++)
						oldLightPos[j] = l.origin[j] + currentLightOffset[j];
					oldLightPos[3] = 1;
					...
					if (numFrozenAxes == 0) { // restrict to same depth plane
						...
						if (numFrozenAxes == 1) {
							...
							var p2 = new float[3];
							var origin = freezeMode == 1 ? l.origin : originalLightPosition;
							for (int j = 0; j < 3; j++)
								p2[j] = origin[j];
							var v2 = new float[3];
							v2[axis] = 1;
					...
					for (int j = 0; j < 3; j++) {
						int offset = (int) (Math.round((newLightPos[j] - l.origin[j]) / gridSize) * gridSize);
						l.offset[j] = offset;
						l.pos[j] = l.origin[j] + offset;
					}
				}
			}
				...
				case GRAB:
					if (numFrozenAxes > 0) {
						...
						for (int i = 0; i < 3; i++)
							point[i] = origin[i];
						point[3] = 1;
						Mat4.projectVec(lightPos, projectionMatrix, point);
						if (point[3] <= 0)
							break;
						...
						for (int i = 0; i < 3; i++) {
							if (!frozenAxes[i]) {
								...
								drawLineSpan(g, lightPos, stepAlongAxis);
							}
						}
					}
					break;
