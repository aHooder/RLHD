...
public class ArHosekSkyModel {
	public static double TERRESTRIAL_SOLAR_RADIUS = toRadians(0.51) / 2;
	...
	public static ArHosekSkyModelState arhosekskymodelstate_alloc_init(
		...
		for (int wl = 0; wl < 11; ++wl) {
			ArHosekSkyModel_CookConfiguration(
				ArHosekSkyModelData_Spectral.datasets[wl],
				state.configs[wl],
			...
			state.radiances[wl] =
				ArHosekSkyModel_CookRadianceConfiguration(
					ArHosekSkyModelData_Spectral.datasetsRad[wl],
					atmospheric_turbidity,
					ground_albedo,
					solar_elevation
				);
	...
	static double blackbody_scaling_factor = 3.19992 * 10E-11;

	//   'art_blackbody_dd_value()' function
	//
	//   Blackbody radiance, Planck's formula

	static double art_blackbody_dd_value(
		double temperature,
		double lambda
	) {
	...
	public static ArHosekSkyModelState arhosek_rgb_skymodelstate_alloc_init(double turbidity, double albedo, double elevation) {
		...
		for (int channel = 0; channel < 3; ++channel) {
			ArHosekSkyModel_CookConfiguration(
				ArHosekSkyModelData_RGB.datasetsRGB[channel],
				state.configs[channel],
				turbidity,
				albedo,
				elevation
			...
			state.radiances[channel] =
				ArHosekSkyModel_CookRadianceConfiguration(
					ArHosekSkyModelData_RGB.datasetsRGBRad[channel],
					turbidity,
					albedo,
					elevation
				);
	...
	static double arhosekskymodel_sr_internal(
		ArHosekSkyModelState state,
		int turbidity,
		int wl,
		double elevation
		...
		int pos =
			(int) (pow(2.0 * elevation / PI, 1.0 / 3.0) * pieces); // floor

		if (pos > 44) pos = 44;

		final double break_x =
			pow(((double) pos / (double) pieces), 3.0) * (PI * 0.5);

		final double[] coefs = ArHosekSkyModelData_Spectral.solarDatasets[wl];
		int coef_offset = order * pieces * turbidity + order * (pos + 1) - 1;
	...
	static double arhosekskymodel_solar_radiance_internal2(
		...
		for (int i = 0; i < 6; i++)
			ldCoefficient[i] =
				(1.0 - wl_frac) * ArHosekSkyModelData_Spectral.limbDarkeningDatasets[wl_low][i]
				+ wl_frac * ArHosekSkyModelData_Spectral.limbDarkeningDatasets[wl_low + 1][i];
	...
	private static double ArHosekSkyModel_GetRadianceInternal(
		double[] configuration,
		double theta,
		double gamma
	) {
		final double expM = exp(configuration[4] * gamma);
		final double rayM = cos(gamma) * cos(gamma);
		final double mieM = (1.0 + rayM) / pow((1.0 + configuration[8] * configuration[8] - 2.0 * configuration[8] * cos(gamma)), 1.5);
		final double zenith = sqrt(cos(theta));
	...
	private static void ArHosekSkyModel_CookConfiguration(
		...
		solar_elevation = pow(solar_elevation / (PI / 2), (1. / 3.));

		// alb 0 low turb

		elev_offset = 9 * 6 * (int_turbidity - 1);
		for (int i = 0; i < 9; ++i) {
			//(1-t).^3* A1 + 3*(1-t).^2.*t * A2 + 3*(1-t) .* t .^ 2 * A3 + t.^3 * A4;
			config[i] =
				(1.0 - albedo) * (1.0 - turbidity_rem)
				* (
					pow(1.0 - solar_elevation, 5.0) * dataset[elev_offset + i] +
					5.0 * pow(1.0 - solar_elevation, 4.0) * solar_elevation * dataset[elev_offset + i + 9] +
					10.0 * pow(1.0 - solar_elevation, 3.0) * pow(solar_elevation, 2.0) * dataset[elev_offset + i + 18] +
					10.0 * pow(1.0 - solar_elevation, 2.0) * pow(solar_elevation, 3.0) * dataset[elev_offset + i + 27] +
					5.0 * (1.0 - solar_elevation) * pow(solar_elevation, 4.0) * dataset[elev_offset + i + 36] +
					pow(solar_elevation, 5.0) * dataset[elev_offset + i + 45]
				);
		}

		// alb 1 low turb
		elev_offset = 9 * 6 * 10 + 9 * 6 * (int_turbidity - 1);
		for (int i = 0; i < 9; ++i) {
			//(1-t).^3* A1 + 3*(1-t).^2.*t * A2 + 3*(1-t) .* t .^ 2 * A3 + t.^3 * A4;
			config[i] +=
		...
		// alb 1 high turb
		elev_offset = 9 * 6 * 10 + 9 * 6 * (int_turbidity);
		for (int i = 0; i < 9; ++i) {
			//(1-t).^3* A1 + 3*(1-t).^2.*t * A2 + 3*(1-t) .* t .^ 2 * A3 + t.^3 * A4;
			config[i] +=
	...
	private static double ArHosekSkyModel_CookRadianceConfiguration(
		...
		// alb 0 low turb
		elev_offset = 6 * (int_turbidity - 1);
		//(1-t).^3* A1 + 3*(1-t).^2.*t * A2 + 3*(1-t) .* t .^ 2 * A3 + t.^3 * A4;
		res = (1.0 - albedo) * (1.0 - turbidity_rem) *
			  (
				  pow(1.0 - solar_elevation, 5.0) * dataset[elev_offset] +
				  5.0 * pow(1.0 - solar_elevation, 4.0) * solar_elevation * dataset[elev_offset + 1] +
				  10.0 * pow(1.0 - solar_elevation, 3.0) * pow(solar_elevation, 2.0) * dataset[elev_offset + 2] +
				  10.0 * pow(1.0 - solar_elevation, 2.0) * pow(solar_elevation, 3.0) * dataset[elev_offset + 3] +
				  5.0 * (1.0 - solar_elevation) * pow(solar_elevation, 4.0) * dataset[elev_offset + 4] +
				  pow(solar_elevation, 5.0) * dataset[elev_offset + 5]
			  );

		// alb 1 low turb
		elev_offset = 6 * 10 + 6 * (int_turbidity - 1);
		//(1-t).^3* A1 + 3*(1-t).^2.*t * A2 + 3*(1-t) .* t .^ 2 * A3 + t.^3 * A4;
		...
		// alb 1 high turb
		elev_offset = 6 * 10 + 6 * (int_turbidity);
		//(1-t).^3* A1 + 3*(1-t).^2.*t * A2 + 3*(1-t) .* t .^ 2 * A3 + t.^3 * A4;
